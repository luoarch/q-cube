...
const SECTOR_MAP: Record<string, { sector: string; subSector: string }> = {
  PETR4: { sector: "Petróleo, Gás e Biocombustíveis", subSector: "Exploração, Refino e Distribuição" },
  PETR3: { sector: "Petróleo, Gás e Biocombustíveis", subSector: "Exploração, Refino e Distribuição" },
...
const PRICE_MAP: Record<string, { price: number; marketCap: number }> = {
  PETR4: { price: 38.50, marketCap: 500_000_000_000 },
  PETR3: { price: 42.10, marketCap: 500_000_000_000 },
  VALE3: { price: 62.30, marketCap: 270_000_000_000 },
  WEGE3: { price: 55.20, marketCap: 140_000_000_000 },
  ITUB4: { price: 33.80, marketCap: 320_000_000_000 },
  BBDC4: { price: 13.90, marketCap: 140_000_000_000 },
  BBAS3: { price: 28.50, marketCap: 160_000_000_000 },
  ABEV3: { price: 13.20, marketCap: 210_000_000_000 },
  B3SA3: { price: 11.50, marketCap: 65_000_000_000 },
  RENT3: { price: 42.80, marketCap: 45_000_000_000 },
  SUZB3: { price: 58.90, marketCap: 80_000_000_000 },
  LREN3: { price: 14.60, marketCap: 14_000_000_000 },
  HAPV3: { price: 3.80, marketCap: 28_000_000_000 },
  RAIL3: { price: 22.10, marketCap: 38_000_000_000 },
  JBSS3: { price: 36.40, marketCap: 82_000_000_000 },
  GGBR4: { price: 18.20, marketCap: 34_000_000_000 },
  CSNA3: { price: 11.30, marketCap: 15_000_000_000 },
  VIVT3: { price: 53.60, marketCap: 90_000_000_000 },
  CMIG4: { price: 12.80, marketCap: 36_000_000_000 },
  ELET3: { price: 44.50, marketCap: 85_000_000_000 },
  EMBR3: { price: 52.30, marketCap: 39_000_000_000 },
  MGLU3: { price: 8.20, marketCap: 5_500_000_000 },
  TOTS3: { price: 30.50, marketCap: 18_000_000_000 },
  RADL3: { price: 26.30, marketCap: 44_000_000_000 },
  EQTL3: { price: 32.10, marketCap: 36_000_000_000 },
};
...
async function main() {
  ...
  for (const asset of allAssets) {
    ...
    if (sectorInfo) {
      ...
      await db.update(assets)
        .set({ sector: defaultSector.sector, subSector: defaultSector.subSector, updatedAt: new Date() })
        .where(eq(assets.id, asset.id));
      sectorUpdated++;
    }
  }
  ...
  const allIssuers = await db.select({ id: issuers.id, legalName: issuers.legalName, sector: issuers.sector }).from(issuers);
  ...
  for (const fs of allFS) {
    ...
    const marketCap = priceInfo
      ? priceInfo.marketCap
      : Math.round(pseudoRandom(ticker.charCodeAt(0) + ticker.charCodeAt(1)) * 50_000_000_000 + 1_000_000_000);

    const netDebt = fs.netDebt ? Number(fs.netDebt) : 0;
    const enterpriseValue = marketCap + netDebt;

    await db.update(financialStatements)
      .set({
