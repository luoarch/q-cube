"""add dividend_yield, net_buyback_yield, net_payout_yield to compat view
...
Migration note:
- DROP + CREATE materialized view (same pattern as all prior compat view migrations)
- Adds 3 columns from computed_metrics: dividend_yield, net_buyback_yield, net_payout_yield
- All existing columns remain unchanged (no semantic change)
- New columns are LEFT JOIN — NULL when metric not computed for an issuer
- View unavailable briefly during DROP+CREATE (not CONCURRENTLY replaceable for mat views)
- Unique index recreated after CREATE
- Deploy order: run migration, then deploy app. App tolerates missing columns (raw SQL queries).
"""
...
# Shared view body (everything except the 3 new NPY columns)
_VIEW_BASE = """
        SELECT
            s.id AS security_id,
            i.id AS issuer_id,
...
_NPY_JOINS = """
        LEFT JOIN computed_metrics cm_dy
            ON cm_dy.issuer_id = i.id AND cm_dy.metric_code = 'dividend_yield'
            AND cm_dy.reference_date = cm_ref.reference_date
        LEFT JOIN computed_metrics cm_nby
            ON cm_nby.issuer_id = i.id AND cm_nby.metric_code = 'net_buyback_yield'
            AND cm_nby.reference_date = cm_ref.reference_date
        LEFT JOIN computed_metrics cm_npy
            ON cm_npy.issuer_id = i.id AND cm_npy.metric_code = 'net_payout_yield'
            AND cm_npy.reference_date = cm_ref.reference_date"""
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        + _VIEW_BASE + _NPY_COLUMNS + _VIEW_FROM + _NPY_JOINS + _WHERE
    )
    op.execute(_INDEX)
