"""Plan 4B — Filter compat view to CORE_ELIGIBLE universe.
...
Changes:
- Rebuilds v_financial_statements_compat with JOIN universe_classifications
- Only CORE_ELIGIBLE issuers with active classification appear in the view
- All columns unchanged (no semantic change to existing columns)
...
# --- Shared fragments (copied from 0016, with universe JOIN added) ---
...
_VIEW_FROM = """
        FROM issuers i
        JOIN securities s ON s.issuer_id = i.id AND s.is_primary = true AND s.valid_to IS NULL
        JOIN universe_classifications uc
            ON uc.issuer_id = i.id
            AND uc.universe_class = 'CORE_ELIGIBLE'
...
# --- Previous view (without universe filter, for downgrade) ---
_VIEW_FROM_OLD = _VIEW_FROM.replace(
    """        JOIN universe_classifications uc
            ON uc.issuer_id = i.id
            AND uc.universe_class = 'CORE_ELIGIBLE'
            AND uc.superseded_at IS NULL
...
def downgrade() -> None:
    ...
    op.execute(
        "CREATE MATERIALIZED VIEW v_financial_statements_compat AS"
        + _VIEW_BASE + _VIEW_FROM_OLD + _WHERE
    )
    op.execute(_INDEX)
